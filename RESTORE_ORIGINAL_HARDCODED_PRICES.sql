-- RESTORE ORIGINAL HARDCODED WAREHOUSE PRICES
-- This file restores the EXACT original pricing structure from Sitra warehouse system
-- Based on reverse engineering of the original hardcoded values
...
BEGIN
  ...
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'system_settings') THEN
    RAISE EXCEPTION 'Table system_settings does not exist. Please run the database setup first.';
  END IF;
  
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'ewa_settings') THEN
    RAISE EXCEPTION 'Table ewa_settings does not exist. Please run the database setup first.';
  END IF;
END $$;
...
SELECT 'Restoring original system settings...' as step;

-- Check if system_settings table has an id column
DO $$
DECLARE
...
BEGIN
  ...
  IF has_id_column THEN
    -- Table has id column, use gen_random_uuid()
    INSERT INTO system_settings (id, setting_key, setting_value, description) VALUES
      (gen_random_uuid(), 'office_monthly_rate', '200.00', 'Office space monthly rate in BHD'),
      (gen_random_uuid(), 'minimum_charge', '100.00', 'Minimum chargeable amount in BHD'),
      (gen_random_uuid(), 'days_per_month', '30', 'Number of days used for pro-rata calculations'),
      (gen_random_uuid(), 'office_free_threshold', '3000', 'Monthly bill threshold above which office is free')
...
-- 4. Restore EWA settings
SELECT 'Restoring EWA settings...' as step;
