...
export default function SitraCalculator() {
  ...
  const loadData = async () => {
    try {
      ...
      // Load ALL pricing rates (not just active ones) for debugging
      const [ratesResult, ewaResult, servicesResult, settingsResult] = await Promise.all([
        supabase.from('pricing_rates').select('*').order('space_type'),
      ...
      console.log('All pricing rates loaded:', ratesResult.data?.length || 0)
      console.log('Active pricing rates:', ratesResult.data?.filter(r => r.active)?.length || 0)
      console.log('Very Short rates:', ratesResult.data?.filter(r => r.tenure === 'Very Short')?.length || 0)
      console.log('Mezzanine rates:', ratesResult.data?.filter(r => r.space_type === 'Mezzanine')?.length || 0)
      console.log('Ground Floor rates:', ratesResult.data?.filter(r => r.space_type === 'Ground Floor')?.length || 0)
      
      // Log specific rates for debugging
      console.log('Very Short rates details:', ratesResult.data?.filter(r => r.tenure === 'Very Short'))
      console.log('Mezzanine rates details:', ratesResult.data?.filter(r => r.space_type === 'Mezzanine'))
      ...
      // Filter to only active rates for the calculator
      const activeRates = ratesResult.data?.filter(r => r.active) || []
      console.log('Filtered to active rates:', activeRates.length)
      
      setPricingRates(activeRates)
  ...
  const calculateQuote = () => {
    try {
      ...
      if (pricingRates.length === 0) {
        setError('No pricing rates available. Please refresh the page or contact administrator.')
        setResult(null)
        return
      }
      
      if (!ewaSettings) {
        setError('EWA settings not configured. Please contact administrator.')
        setResult(null)
        return
      }
        ...
        <div className="container mx-auto px-4 py-6">
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div>
                  ...
                  <h2 className="text-3xl font-bold text-gray-900 mb-4">Warehouse Calculator</h2>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    {result.tenure === 'Long' && (
                      <div className="mt-2 p-2 bg-green-100 border border-green-300 rounded text-xs text-green-800">
                        ✅ <strong>Long-term benefit:</strong> You're getting the best monthly rates for your commitment!
                      </div>
                    )}
                  </div>
                    ...
                    <div className="bg-green-50 p-4 rounded-lg border-2 border-green-200">
                      ...
                      {result.tenure === 'Long' && (
                        <div className="mt-2 text-xs text-green-700">
                          💡 This is your monthly rate - Long-term commitment gives you the best value!
                        </div>
                      )}
                    </div>
